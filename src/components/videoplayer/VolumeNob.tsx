...
interface Iprop {
  volume: number;
  volumeChanger: (vo: number) => void;
}
export default function VolumeNob({ volume, volumeChanger }: Iprop) {
  useEffect(() => {
    console.log(volume)
  }, [volume])
  const muteunmut = () => {
  ...
  return (
    <div className="h-full w-full text-white cursor-pointer" onClick={muteunmut} >
      {volume > 0.75 ? (
        <svg
          viewBox="0 0 24 24"
          fill="none"
